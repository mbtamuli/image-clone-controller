...
func (r *DaemonsetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		newSecret, err := newDockerCfgSecret(req.Namespace, req.Name, registry, registryUsername, registryPassword)
		if err != nil {
			log.Error(err, "Failed to create secret object")
			return ctrl.Result{}, err
		}
		log.Info("Creating a Secret for daemonset", "Secret.Namespace", newSecret.Namespace, "Secret.Name", newSecret.Name)
		if err := r.Create(ctx, newSecret); err != nil {
			return ctrl.Result{}, err
		}
		daemonset.Spec.Template.Spec.ImagePullSecrets = []corev1.LocalObjectReference{{Name: newSecret.Name}}
